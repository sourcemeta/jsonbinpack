...
auto hmac_sha384_digest(const std::string_view key,
  ...
  // SHA-384 operates on 128-byte blocks (FIPS 180-4 Section 5.1.2), which is
  ...
  if (key.size() > block_size) {
    const auto digest{sha384_digest(key)};
  ...
  const auto inner_digest{sha384_digest(inner_input)};
  outer_input.append(reinterpret_cast<const char *>(inner_digest.data()),
                     inner_digest.size());
  return sha384_digest(outer_input);
}
