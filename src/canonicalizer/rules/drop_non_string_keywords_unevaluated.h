...
///
/// If the `type` keyword from the Validation is set to `string` and
/// the Unevaluated vocabulary is in use, then keywords from the
/// Unevaluated vocabulary that do not apply to string JSON instances can be
/// removed.
///
/// \f[\frac{S.type = string \land K \subseteq dom(S) }{S
