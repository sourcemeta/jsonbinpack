...
TEST(decoder, zigzag_int_0_0) {
  const int value = 0;
  EXPECT_EQ(sourcemeta::jsonbinpack::decoder::zigzag(value), 0);
}

TEST(decoder, zigzag_int_1_minus_1) {
  const int value = 1;
  EXPECT_EQ(sourcemeta::jsonbinpack::decoder::zigzag(value), -1);
}
...
TEST(decoder, zigzag_int_3_minus_2) {
  const int value = 3;
  EXPECT_EQ(sourcemeta::jsonbinpack::decoder::zigzag(value), -2);
}
