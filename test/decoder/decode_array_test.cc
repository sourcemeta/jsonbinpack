...
TEST(Decoder, BOUNDED_TYPED_ARRAY_true_false_true__no_prefix_encodings) {
  using namespace sourcemeta::jsonbinpack;
  InputByteStream<char> stream{0x03, 0x01, 0x00, 0x01};
  Decoder decoder{stream};
  ...
  const sourcemeta::jsontoolkit::JSON result{decoder.BOUNDED_TYPED_ARRAY(
      {0,
       3,
       options::wrap(options::BYTE_CHOICE_INDEX{std::move(choices)}),
       {}})};
...
TEST(Decoder, BOUNDED_TYPED_ARRAY_true_false_true__same_max_min) {
  using namespace sourcemeta::jsonbinpack;
  InputByteStream<char> stream{0x00, 0x01, 0x00, 0x01};
  Decoder decoder{stream};
...
TEST(Decoder, BOUNDED_TYPED_ARRAY_true_false_5__1_3) {
  using namespace sourcemeta::jsonbinpack;
  InputByteStream<char> stream{0x02, 0x01, 0x00, 0x05};
  Decoder decoder{stream};
...
TEST(Decoder, BOUNDED_TYPED_ARRAY_complex) {
  using namespace sourcemeta::jsonbinpack;
  InputByteStream<char> stream{0x03, 0x01, 0x01, 0x66, 0x6f, 0x6f, 0xfa, 0x01};
  Decoder decoder{stream};
